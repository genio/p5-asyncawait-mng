...
following arguments, rather than attributes, can be passed into the constructor.
...
C<AF_UNIX> is for the unix domain socket and is handled via L<IO::Socket::UNIX>.
C<AF_UNIX> sockets are bound to the file system as their address name space.
...
Listen should be provided an integer value or left unset.
...
=head2 Proto
    ...
    my $sock = IO::Socket->new(..., Proto => IO::Socket::IPPROTO_TCP);
    $sock = IO::Socket->new(..., Proto => 'tcp');

The IP protocol used for this socket. It can either be the string name or the
numeric value in the constants (C<'tcp'>, C<IPPROTO_TCP>, C<'udp'>, or
C<IPPROTO_UDP>). If this argument is left undefined, either C<getaddrinfo> or
the kernel will choose an appropriate value.

=head2 Timeout

    my $sock = IO::Socket->new(..., Timeout => 5);

The timeout value, in seconds, for this socket connection.

=head2 Type

    my $sock = IO::Socket->new(..., Type => IO::Socket::SOCK_STREAM);

The socket type that will be passed along to C<getaddrinfo>. These are usually
C<SOCK_STREAM>, C<SOCK_DGRAM>, or C<SOCK_RAW>. If this argument is left
undefined, C<getaddrinfo> may attempt to infer the type from the service name.

For example, you'll usually use C<SOCK_STREAM> with a C<tcp> connection and
C<SOCK_DGRAM> with a C<udp> connection.

C<SOCK_RAW> sockets allow IP protocols to be implemented in user space. A
...
=head2 new
    ...
    $sock = IO::Socket->new(
        ...
        Type => IO::Socket::SOCK_STREAM, # SOCK_STREAM, SOCK_DGRAM, SOCK_RAW
        Proto => 'tcp', # 'tcp', 'udp', IPPROTO_TCP, IPPROTO_UDP
        # and so on...
    );

Creates an C<IO::Socket>, which is a reference to a newly created symbol (see
the L<Symbol> package). C<new> optionally takes arguments, these arguments are
defined in L<IO::Socket/"CONSTRUCTOR ARGUMENTS">.
...
proper subclass for the domain family is registered. All other arguments will be
passed to the L<IO::Socket/"configuration"> method of the package for that
domain.
...
The timeout in the package can be specified as zero to effect a C<poll>,
but you shouldn't do that because a new L<IO::Select> object will be
created behind the scenes just to do the single poll.  This is
horrendously inefficient.  Use rather true C<select()> with a zero
...
L<IO::Socket/"close">. Only at that point does this function return C<undef>.
...
C<SO_LINGER> enabled with a zero timeout, then the peer's C<close> will generate
a C<RST> segment. Upon receipt of that segment, the local TCP transitions
immediately to B<CLOSED>, and in that state, this method I<will> return
C<undef>.
...
this; returns an empty string otherwise. If there's an error, returns C<undef>.
This call is actually implemented in terms of the C<recvfrom(2)> system call.

Flags are ORed together values, such as C<MSG_BCAST>, C<MSG_OOB>, C<MSG_TRUNC>.
...
Sends a message on a socket. Attempts to send the scalar message to the socket.
Takes the same flags as the system call of the same name. On unconnected
sockets, you must specify a destination to send to, in which case it does a
C<sendto(2)> syscall. Returns the number of characters sent, or C<undef> on
error. The C<sendmsg(2)> syscall is currently unimplemented.
...
=head2 shutdown
    ...
    $sock->shutdown(0); # we stopped reading data
    $sock->shutdown(1); # we stopped writing data
    $sock->shutdown(2); # we stopped using this socket
...
L<IO::Socket/"close"> because it also disables the file descriptor in any
...
Opens a socket of the specified kind and returns it. Domain, type, and protocol
are specified the same as for the syscall of the same name.

=head2 socketpair

    my ($read, $write) = $sock->socketpair(AF_UNIX, SOCK_STREAM, PF_UNSPEC);
    ($read, $write) = IO::Socket::UNIX->socketpair(AF_UNIX, SOCK_STREAM, PF_UNSPEC);

Will return a list of two sockets created, or an empty list on failure.

Differs slightly from C<socketpair> in L<perlfunc> in that the argument list is
a bit simpler.
...
Returns the packed C<sockaddr> address of this end of the connection, in case
you don't know the address because you have several different IPs that the
connection might have come in on. It's the same as C<getsockname(2)>.
...
Unified method to both set and get options in the C<SOL_SOCKET> level. If called
with one argument then L<IO::Socket/"getsockopt"> is called, otherwise
